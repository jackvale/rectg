...
function init() {
    ...
    document.querySelectorAll('.card').forEach((card) => {
        ...
        card.querySelector('.card-action-primary')?.addEventListener('click', () => {
            window.rectgTrack?.('telegram_click', {
                resource_id: id,
                type: card.getAttribute('data-type') || '',
            });
        });
        card.querySelector('.card-title, .card-action[href^="/p/"]')?.addEventListener('click', () => {
            window.rectgTrack?.('resource_detail', { resource_id: id });
        });
    });
}
