...
用法:
    python3 crawler/crawl.py              # 爬新链接，并刷新 30 天前的旧数据
    python3 crawler/crawl.py --limit 10   # 只爬前 10 个（测试用）
    python3 crawler/crawl.py --new        # 只爬新链接，不刷新旧数据
    python3 crawler/crawl.py --older-than-days 7  # 刷新 7 天前的旧数据
    python3 crawler/crawl.py --no-resume  # 清空 entries 表，从头开始
"""
...
def main():
    ...
    if not DB_PATH.exists():
        ...
        log.error("   请先运行: python3 crawler/parse_links.py")
        sys.exit(1)
    ...
    if not links:
        if args.new:
            ...
            log.error("❌ links 表为空，请先运行: python3 crawler/parse_links.py")
        else:
